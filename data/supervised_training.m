...
function [theta_day, theta_night, history] = train_gnss_weight_model(sat_xyzlist, ttx, ref_pos_ecef)
    % 입력: 
    %   sat_xyzlist: [Sat_Pos, Sat_Num, [Az, El], [IPP_Lat, IPP_Lon, LocalTime, isDay]]
    %   ttx: 위성 시각 및 보정된 의사거리 데이터
    %   ref_pos_ecef: 참값 위치 [X, Y, Z]
    ...
    % 초기 파라미터 [a, b, n] 설정
    theta_day = [0.1,0.1, 2.0];   
    theta_night = [0.1, 0.1, 2.0]; 
    
    lr_ab = 0.1;      % 학습률 (Learning Rate)
    lr_n = 0.001;
    lr_vec = [lr_ab,lr_ab,lr_n];
    max_iter = 1000;    % 전체 데이터 반복 횟수 (Epochs)
    ...
    for iter = 1:max_iter
        ...
        theta_day = theta_day - lr_vec.* (grad_day_total / num_train);
        theta_night = theta_night - lr_vec.* (grad_night_total / num_train);
