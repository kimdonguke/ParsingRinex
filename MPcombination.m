...
function final_MP = applyDetrending(fullTable, raw_MP, method_vec)
    % 위성별, Arc별 바이어스 제거 (Mean Removal) + Cycle Slip 감지
    ...
    for k = 1:length(uArcs)
        ...
        % [수정] 데이터 개수가 너무 적으면(예: 10개 미만) 통계적으로 유의미하지 않으므로 NaN 처리
        if sum(currArc) < 10 
            detrended_sub(currArc) = NaN; 
            continue; 
        end
